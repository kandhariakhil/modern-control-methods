...
def simulate(cat_states, cat_controls, t, step_horizon, N, reference, save, obs_x, obs_y, obs_dia):
    def create_triangle(state=[0,0,0], h=0.2, w=0.2, update=False):
        x, y, th = state
        triangle = np.array([
            [h, 0   ],
            [0,  w/2],
    ...
    def draw_circle(obs_x, obs_y, obs_dia, color='red', linewidth=2, fill=False):
        # Create a circle patch
        circle = patches.Circle((obs_x, obs_y), obs_dia/2, edgecolor=color, linewidth=linewidth, fill=fill)
    ...
    obstacle_circle = draw_circle(obs_x,obs_y,obs_dia)
